...
pub use crate::lapce_core::syntax::Syntax;
use crate::{
    lapce_core::{
    ...
    lapce_rpc::{plugin::PluginId, proxy::ProxyResponse},
};
...
impl LapceEditorBufferData {
    ...
    fn inline_find(
        ...
        if let Some(new_index) = match direction {
            ...
            self.run_move_command(
                ctx,
                &crate::lapce_core::movement::Movement::Offset(
    ...
    pub fn apply_completion_item(&mut self, item: &CompletionItem) -> Result<()> {
        ...
        let additional_edit: Vec<_> = item
            ...
            .flatten()
            .map(|edit| {
                let selection = crate::lapce_core::selection::Selection::region(
                    self.doc.buffer().offset_of_position(&edit.range.start),
                    self.doc.buffer().offset_of_position(&edit.range.end),
                );
                (selection, edit.new_text.as_str())
            })
            .collect::<Vec<(crate::lapce_core::selection::Selection, &str)>>();
                ...
                CompletionTextEdit::Edit(edit) => {
                    ...
                    match text_format {
                        ...
                        lsp_types::InsertTextFormat::SNIPPET => {
                            ...
                            let mut selection =
                                crate::lapce_core::selection::Selection::new();
                            let (_tab, (start, end)) = &snippet_tabs[0];
                            let region =
                                crate::lapce_core::selection::SelRegion::new(
                                    *start, *end, None,
                                );
    ...
    fn page_move(&mut self, ctx: &mut EventCtx, down: bool, mods: Modifiers) {
        ...
        self.run_move_command(
            ctx,
            if down {
                &crate::lapce_core::movement::Movement::Down
            } else {
                &crate::lapce_core::movement::Movement::Up
            },
            Some(lines),
            mods,
        );
    ...
    fn run_move_command(
        &mut self,
        ctx: &mut EventCtx,
        movement: &crate::lapce_core::movement::Movement,
        count: Option<usize>,
        mods: Modifiers,
    ) -> CommandExecuted {
    ...
    fn run_edit_command(
        ...
        let cursor = &mut Arc::make_mut(&mut self.editor).cursor;
        let yank_data =
            if let crate::lapce_core::cursor::CursorMode::Visual { .. } =
                &cursor.mode
            {
        ...
        match cmd {
            ...
            SearchWholeWordForward => {
                ...
                if let Some((start, _end)) = next {
                    self.run_move_command(
                        ctx,
                        &crate::lapce_core::movement::Movement::Offset(start),
                        None,
                        mods,
                    );
                }
            ...
            JumpToNextSnippetPlaceholder => {
                if let Some(snippet) = self.editor.snippet.as_ref() {
                    ...
                    if let Some((_, (start, end))) = snippet.get(current + 1) {
                        let mut selection =
                            crate::lapce_core::selection::Selection::new();
                        let region = crate::lapce_core::selection::SelRegion::new(
                            *start, *end, None,
                        );
                        selection.add_region(region);
            ...
            Search => {
                ...
                let region = match &self.editor.cursor.mode {
                    crate::lapce_core::cursor::CursorMode::Normal(offset) => {
                        crate::lapce_core::selection::SelRegion::caret(*offset)
                    }
                    crate::lapce_core::cursor::CursorMode::Visual {
                        start,
                        end,
                        mode: _,
                    } => crate::lapce_core::selection::SelRegion::new(
                    ...
                    crate::lapce_core::cursor::CursorMode::Insert(selection) => {
...
fn apply_edit(main_split: &mut LapceMainSplitData, path: &Path, edits: &[TextEdit]) {
    ...
    main_split.edit(path, &edits, crate::lapce_core::editor::EditType::Other);
}
